...
class Individual:
  ...
  def __init__(self, score = float("inf"), win_percentage = 0, actions = [], food_count = 55, capsules_count = 2, actions_count = 0):
  ...
  def fitness(self):
    return self.score + self.win_percentage + self.actions_count - (self.capsules_count + self.food_count)
...
class Generation:
  ...
  def performance(self):
    ...
    print 'Best Score: {best_score}'.format(best_score=self.best_score().score)
    print 'Worst Score: {worst_score}'.format(worst_score=self.worst_score().score)
    print 'Avg Score: {avg_score}'.format(avg_score=sum(ind.score for ind in self.individuals) / len(self.individuals))
    print 'Best Fitness: {best_fitness}'.format(best_fitness=self.best_fitness().fitness())
    print 'Worst Fitness: {worst_fitness}'.format(worst_fitness=self.worst_fitness().fitness())
    print 'Avg Fitness: {avg_fitness}'.format(avg_fitness=sum(ind.fitness() for ind in self.individuals) / len(self.individuals))
